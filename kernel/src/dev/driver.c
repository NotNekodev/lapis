...
static int match_device(const device_t *dev, const device_id_t *id) {
    if (id->class && id->class != dev->class) return 0;
    if (id->subclass && id->subclass != dev->subclass) return 0;
    if (id->prog_if && id->prog_if != dev->prog_if) return 0;
    if (id->vendor_id && id->vendor_id != dev->vendor_id) return 0;
    if (id->device_id && id->device_id != dev->device_id) return 0;
    return 1;
}
    ...
    for (driver_t *drv = driver_list; drv; drv = drv->next) {
        for (size_t i = 0; i < drv->id_count; i++) {
            if (match_device(dev, &drv->id_table[i])) {
                ...
                if (drv->irq_handler && dev->irqs) {
                    for (device_irq_t *i = dev->irqs; i; i = i->next) {
